...
final class BockProfilePrefsTests: XCTestCase {
    ...
    private func assumeTwoMembers() throws {
        let members: [ProfileTestAPI.HouseholdMember]
        do {
            members = try awaitResult { try await ProfileTestAPI.householdMembers() }
        } catch {
            throw XCTSkip("Server config required for profile tests: \(error)")
        }
